...
export function investigationViews(state) {
  ...
  function initial(id) {
    ...
    return `<section class="panel initial-panel"><div class="eyebrow">先独立判断</div><h2>${attempted(id) ? '已保存的初判' : '打开分析结果前，留下你的判断'}</h2><label class="answer-prompt" for="initial-${id}">${a.hypothesis}</label><textarea id="initial-${id}" data-initial="${id}" rows="3" placeholder="写下你的初步判断和验证方法。它可以是错的。">${esc(n.initial || '')}</textarea>${id === 'q1' ? `<div class="initial-calcs">${[['cpc', 'CPC（USD / click）'], ['cpa', 'CPA（USD / booking）'], ['roas', 'ROAS（倍）']].map(([key, label]) => `<label>${label}<input type="number" min="0" step="any" data-initialcalc="${key}" value="${esc(n.calcs?.[key] ?? '')}" placeholder="先独立计算"></label>`).join('')}</div><p class="small">先用原始文件独立核算这三个指标。允许计算错误，保存后再逐项核对。</p>` : ''}<div class="answer-footer"><span class="small">本地学习记录，不向讲师自动提交。</span><button class="button primary" data-action="save-initial" data-id="${id}">保存初判，进入分析 →</button></div></section>`;
  }
  function chain() {
