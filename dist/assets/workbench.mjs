...
function render(){
 ...
 document.querySelector('#breadcrumb').textContent=({overview:'数据与课程总览',brief:'案例故事与题目依据',data:'数据与口径',notes:'我的分析记录'}[r]||lessons.find(l=>l.id===r)?.title);
...
function applyDataset(data,bytes,name){state.data=data;rawBytes=bytes;fileName=name;state.publisher='';state.campaign='';state.query='';state.minClicks=0;state.page=0;state.tablePage=0;state.budgets={};controller.dataLoaded();render();toast(`已载入 ${num(data.rows.length)} 条记录。`)}
...
async function loadCaseMirror(useLocal){
 ...
 try{
  ...
  const sheets=wb.SheetNames.map(name=>({name,startRow:wb.Sheets[name]['!ref']?XLSX.utils.decode_range(wb.Sheets[name]['!ref']).s.r:0,matrix:XLSX.utils.sheet_to_json(wb.Sheets[name],{header:1,defval:null})}));
