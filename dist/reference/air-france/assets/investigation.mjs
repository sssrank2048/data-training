...
// Reproducible evidence for the narrative and worked solutions. Thresholds are descriptive baselines.
export function courseEvidence(data) {
 if(!data?.rows?.length)return null;
 const total=aggregate(data.rows);
 const channels=groupRows(data.rows,'publisher').map(g=>({...g,costShare:divide(g.cost,total.cost),revenueShare:divide(g.revenue,total.revenue),bookingShare:divide(g.bookings,total.bookings),quadrant:`${g.cpc===null?'CPC 未知':g.cpc<=total.cpc?'低 CPC':'高 CPC'} / ${g.cvr===null?'CVR 未知':g.cvr>=total.cvr?'高 CVR':'低 CVR'}`}));
 const leader=channels[0],best=[...channels].filter(g=>g.roas!==null).sort((a,b)=>b.roas-a.roas)[0];
 const focalRows=data.rows.filter(r=>r.publisher===leader.name);
 const campaigns=groupRows(focalRows,'campaign');
 const rankedFocus=campaigns.slice(0,3).filter(g=>g.roas!==null).sort((a,b)=>b.roas-a.roas);
 const campaignContrast={strong:rankedFocus[0],weak:rankedFocus.at(-1)};
 const strategies=groupRows(focalRows.map(r=>({...r,strategy:`${r.matchType} / ${r.bidStrategy}`})),'strategy');
 const bySpend=(a,b)=>b.cost-a.cost||a.sourceRow-b.sourceRow;
 const strong=[...focalRows].filter(r=>r.bookings>0&&r.clicks>0&&r.revenue>r.cost).sort((a,b)=>b.bookings-a.bookings||bySpend(a,b))[0];
 const weak=[...focalRows].filter(r=>r.bookings>0&&r.revenue<r.cost&&r.cost>0&&r.status==='Live').sort(bySpend)[0];
 const zero=[...focalRows].filter(r=>r.bookings===0&&r.cost>0).sort(bySpend)[0];
 const actions=[strong&&{kind:'protect',row:strong},weak&&{kind:'review',row:weak},zero&&{kind:'zero',row:zero}].filter(Boolean).map(a=>({...a,metrics:aggregate([a.row])}));
 const zeroTotal=aggregate(data.rows.filter(r=>r.bookings===0));
 const activeTotal=aggregate(data.rows.filter(r=>r.bookings>0));
 const anomalies=data.rows.filter(r=>r.clicks===0||r.bookings>r.clicks);
 const comparableCpc=data.rows.filter(r=>r.clicks>0);
 const simpleCpc=divide(comparableCpc.reduce((sum,r)=>sum+r.cost/r.clicks,0),comparableCpc.length);
 return {total,channels,leader,best,campaigns,campaignContrast,strategies,actions,zeroTotal,activeTotal,anomalies,simpleCpc,kayak:data.kayak?aggregate([data.kayak]):null};
}
