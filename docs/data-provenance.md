...
- 预算页是固定效率情景。未来 CPC/CVR 变化由学员显式输入，既不是观测数据，也不是扩量预测或广告因果增量。

复核可运行 `node scripts/inspect-data.mjs`。解析规则与网站共用模块，真实原文件亦纳入本地测试。

## 经营调查进阶版

第二版增加品牌词规则分类、共同结构比较、CPA 对称分解、门槛敏感性和同预算压力测试。均由原数据或明确标注的分析规则/假设派生，未添加虚构经营记录。方法详见 [分析方法](analysis-methods.md)。
