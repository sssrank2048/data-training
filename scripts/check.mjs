...
let errors=[];
for(const file of [...files('dist'),...files('scripts'),...files('tests')].filter(f=>f.endsWith('.mjs'))){
...
for(const html of files('dist').filter(f=>f.endsWith('.html')))for(const match of fs.readFileSync(html,'utf8').matchAll(/(?:src|href)="(\/[^"#]+)"/g))if(!fs.existsSync('dist'+match[1]))errors.push('Missing asset: '+match[1]);
